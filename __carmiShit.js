...
function isBlocked2$8($tracked, key, val, context) {
    let $cond_10 = 0;
    const res = ((($cond_10 = 1) && val)&&(($cond_10 = 2) && !($topLevel[2 /*"$mapValues_carmiShit_5_25_2"*/][val])));
    (($cond_10) >= (2) ) &&  trackPath($tracked, [$topLevel[2 /*"$mapValues_carmiShit_5_25_2"*/],val]);
    return res;
  }
...
function canBeWorkedOn$20($tracked, key, val, context) {
    let $cond_25 = 0;
    const res = ((!(val["done"]))&&(((($cond_25 = 1) && !(val["blockedBy"]))||(($cond_25 = 2) && $topLevel[2 /*"$mapValues_carmiShit_5_25_2"*/][val["blockedBy"]]))));
    (($cond_25) >= (2) ) &&  trackPath($tracked, [$topLevel[2 /*"$mapValues_carmiShit_5_25_2"*/],val["blockedBy"]]);
    return res;
  }
...
function $mapValues_carmiShit_5_25_2Build($tracked) {
    ...
    const builderFunctions = [$mapValues_carmiShit_4_25_1Build,$isBlockedBuild,$mapValues_carmiShit_5_25_2Build,$isBlocked2Build,$pendingTodosBuild,$isBlocked3Build,$canBeWorkedOnBuild,$shownTodoBuild,$blockedGroupedBuild];
  const builderNames = ["$mapValues_carmiShit_4_25_1","isBlocked","$mapValues_carmiShit_5_25_2","isBlocked2","pendingTodos","isBlocked3","canBeWorkedOn","shownTodo","blockedGrouped"];
