var model = (function () {
    ...
    const carmiFunction = model;

    let intoTimeMachineHolder = () => {};

    let intoTimeMachineGetterChange = () => {};
    ...
    if (/* DEBUG */true) {
      ...
      if (typeof _ === 'function') {
        timeMachine.initialState = _.cloneDeep($model);
      } else {
        timeMachine.initialState = $model;
      }

      intoTimeMachineHolder = (path, value) => timeMachineHolder.push({
        path,
        value: _.cloneDeep(value)
      });

      intoTimeMachineGetterChange = (key, oldValue, newValue) => {
        getterChange[key] = {
      ...
      function snapShot() {
        ...
        if (timeMachineHolder.length) {
          ...
          for (entry of timeMachineHolder) {
            let curr = res;
            let key;

            for (_key of entry.path) {
              if (key) curr = curr[key];
              key = _key;
              if (!curr[key]) curr[key] = {};
            }

            curr[key] = entry.value;
          }

          timeMachine.push({
            instructions: timeMachineHolder,
            getterChange,
            res
          });
          timeMachineHolder = [];
  ...
  const debug = () => {
    ...
    console.log(JSON.stringify(serialize(all, {
      $trackingMap,
      $invalidatedMap
    }), null, 2));
  };
  ...
  function splice(pathWithKey, len, ...newItems) {
    ...
    let $cond_2 = 0;
    const res = ((($cond_2 = 1) && val)&&(($cond_2 = 2) && !($model["todos"][val]["done"])));
    (($cond_2) >= (2) ) &&  trackPath($tracked, [$model,"todos",val,"done"]);
...
function $isBlockedBuild($tracked) {
    ...
    checkTypes($topLevel[0 /*"blockedBy"*/], 'isBlocked', ["object"], 'mapValues', '../tmp/carmi-playground/carmiShit.carmi.js:16:29')
    const newValue = mapValuesOpt($tracked, '58', isBlocked$0, $topLevel[0 /*"blockedBy"*/], null, true);
     trackPath($tracked, [$topLevel,0]);

    return newValue;
  }

function isBlocked2$8($tracked, key, val, context) {
    let $cond_10 = 0;
    const res = ((($cond_10 = 1) && val)&&(($cond_10 = 2) && !($topLevel[2 /*"$mapValues_carmiShit_5_25_1"*/][val])));
    (($cond_10) >= (2) ) &&  trackPath($tracked, [$topLevel[2 /*"$mapValues_carmiShit_5_25_1"*/],val]);
...
function $isBlocked2Build($tracked) {
    ...
    checkTypes($topLevel[0 /*"blockedBy"*/], 'isBlocked2', ["object"], 'mapValues', '../tmp/carmi-playground/carmiShit.carmi.js:17:30')
    const newValue = mapValuesOpt($tracked, '8', isBlocked2$8, $topLevel[0 /*"blockedBy"*/], null, true);
     trackPath($tracked, [$topLevel,0]);
...
function $isBlocked3Build($tracked) {
    ...
    checkTypes($topLevel[0 /*"blockedBy"*/], 'isBlocked3', ["object"], 'mapValues', '../tmp/carmi-playground/carmiShit.carmi.js:18:30')
    const newValue = mapValuesOpt($tracked, '15', isBlocked35, $topLevel[0 /*"blockedBy"*/], null, true);
     trackPath($tracked, [$topLevel,0]);

    return newValue;
  }

function blockedBy$20($tracked, key, val, context) {
...
function $blockedByBuild($tracked) {
    

    checkTypes($model["todos"], 'blockedBy', ["object"], 'mapValues', '../tmp/carmi-playground/carmiShit.carmi.js:4:25')
    const newValue = mapValuesOpt($tracked, '20', blockedBy$20, $model["todos"], null, true);
     trackPath($tracked, [$model,"todos"]);

    return newValue;
  }

function canBeWorkedOn$24($tracked, key, val, context) {
    let $cond_26 = 0;let $cond_29 = 0;
    const res = ((($cond_26 = 1) && !(val["done"]))&&(($cond_26 = 2) && ((($cond_29 = 1) && !(val["blockedBy"]))||(($cond_29 = 2) && $topLevel[2 /*"$mapValues_carmiShit_5_25_1"*/][val["blockedBy"]]))));
    (($cond_29) >= (2) ) &&  trackPath($tracked, [$topLevel[2 /*"$mapValues_carmiShit_5_25_1"*/],val["blockedBy"]]);
...
function $canBeWorkedOnBuild($tracked) {
    ...
    checkTypes($model["todos"], 'canBeWorkedOn', ["object"], 'mapValues', '../tmp/carmi-playground/carmiShit.carmi.js:21:29')
    const newValue = mapValuesOpt($tracked, '24', canBeWorkedOn$24, $model["todos"], null, true);
     trackPath($tracked, [$model,"todos"]);
...
function $shownTodoBuild($tracked) {
    let $cond_36 = 0;let $cond_37 = 0;

    
    const newValue = ((($cond_36 = 1) && ((($cond_37 = 1) && $model["showCompleted"])&&(($cond_37 = 2) && $topLevel[6 /*"canBeWorkedOn"*/])))||(($cond_36 = 2) && $topLevel[4 /*"pendingTodos"*/]));
    (($cond_36) >= (2) ) &&  trackPath($tracked, [$topLevel,4]);
(($cond_37) >= (2) ) &&  trackPath($tracked, [$topLevel,6]);
 trackPath($tracked, [$model,"showCompleted"]);

    return newValue;
  }

function pendingTodos$41($tracked, key, val, context) {
...
function $pendingTodosBuild($tracked) {
    ...
    checkTypes($model["todos"], 'pendingTodos', ["object"], 'filterBy', '../tmp/carmi-playground/carmiShit.carmi.js:3:28')
    const newValue = filterByOpt($tracked, '41', pendingTodos$41, $model["todos"], null, true);
     trackPath($tracked, [$model,"todos"]);

    return newValue;
  }

function blockedGrouped$46$48($tracked, key, val, context) {
...
function blockedGrouped$46($tracked, key, val, context) {
    
    const res = filterByOpt($tracked, '48', blockedGrouped$46$48, $model["todos"], array($tracked,[key],'48arr',1,true), true);
     trackPath($tracked, [$model,"todos"]);
...
function $blockedGroupedBuild($tracked) {
    ...
    checkTypes($topLevel[4 /*"pendingTodos"*/], 'blockedGrouped', ["object"], 'mapValues', '../tmp/carmi-playground/carmiShit.carmi.js:33:37')
    const newValue = mapValuesOpt($tracked, '46', blockedGrouped$46, $topLevel[4 /*"pendingTodos"*/], null, true);
     trackPath($tracked, [$topLevel,4]);

    return newValue;
  }

function $mapValues_carmiShit_5_25_1$54($tracked, key, val, context) {
...
function $mapValues_carmiShit_5_25_1Build($tracked) {
    

    checkTypes($model["todos"], '$mapValues_carmiShit_5_25_1', ["object"], 'mapValues', '../tmp/carmi-playground/carmiShit.carmi.js:5:25')
    const newValue = mapValuesOpt($tracked, '54', $mapValues_carmiShit_5_25_1$54, $model["todos"], null, true);
     trackPath($tracked, [$model,"todos"]);
    ...
    $blockedByBuild,$isBlockedBuild,$mapValues_carmiShit_5_25_1Build,$isBlocked2Build,$pendingTodosBuild,$isBlocked3Build,$canBeWorkedOnBuild,$shownTodoBuild,$blockedGroupedBuild
  ];
  const builderNames = [
    "blockedBy","isBlocked","$mapValues_carmiShit_5_25_1","isBlocked2","pendingTodos","isBlocked3","canBeWorkedOn","shownTodo","blockedGrouped"
  ];
  ...
  function updateDerived() {
    for (let i = 0; i < 9; i++) {
      if ($first || $invalidatedRoots.has(i)) {
        ...
        if (builderNames[i]) {
          debugger;
          !/^\$/.test(builderNames[i]) && intoTimeMachineGetterChange(builderNames[i], $res[builderNames[i]], newValue);
...
$tainted = new WeakSet();
    ...
    if (/* DEBUG */true) {
      Object.assign($res, {
        ...
        $source: () => null,
        timeMachine,
        viewStateFromTimeMachine: timeMachineIdx => {
          const res = typeof _ === 'function' ? _.cloneDeep(timeMachine.initialState) : timeMachine.initialState;
          let gettersResult = {};

          for ({
            instructions,
            getterChange
          } of [...timeMachine].splice(0, timeMachineIdx)) {
            if (getterChange) gettersResult = { ...gettersResult,
              ...getterChange
            };
            ...
            if (instructions.path) {
              ...
              curr[key] = instructions.res;
            }
          }

          const retVal = carmiFunction(res, $funcLibRaw, $batchingStrategy);
          return { ...retVal,
            ...gettersResult
          };
        }
